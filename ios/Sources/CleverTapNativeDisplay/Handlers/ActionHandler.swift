...
class ActionHandler {
    ...
    func handleAction(
        ...
        Task { @MainActor in
            do {
                NDLogger.d("ActionHandler", "Handling action for node: \(nodeId)")
                ...
                guard shouldProceed else {
                    NDLogger.d("ActionHandler", "Component listener consumed interaction for node: \(nodeId)")
                    return
                }
                ...
                NDLogger.e("ActionHandler", "Error handling action for node: \(nodeId), error: \(error)")
    ...
    func handleLifecycleAction(
        ...
        Task { @MainActor in
            do {
                NDLogger.d("ActionHandler", "Handling lifecycle action for node: \(nodeId)")
                switch action {
                case .openUrl(let openUrlAction):
                    try await handleOpenUrl(openUrlAction, nodeId: nodeId)
                case .custom(let customAction):
                ...
                NDLogger.e("ActionHandler", "Error handling lifecycle action for node: \(nodeId) - \(error)")
                actionListener?.onActionError(action: action, error: error)
            }
        }
    }
    ...
    func fireSystemEvent(eventName: String, properties: [String: Any]? = nil, deduplicate: Bool = false) {
        if deduplicate {
            guard firedSystemEvents.insert(eventName).inserted else {
                NDLogger.d("ActionHandler", "System event already fired, skipping: \(eventName)")
                return
            }
        }
        Task { @MainActor in
            NDLogger.d("ActionHandler", "Firing system event: \(eventName)")
    ...
    func handleInteractionWithoutAction(
        ...
        Task { @MainActor in
            NDLogger.d("ActionHandler", "Handling interaction without action for node: \(nodeId)")
    ...
    private func notifyComponentListener(
        ...
        if consumed {
            NDLogger.d("ActionHandler", "Component listener consumed interaction for: \(nodeId)")
        }
    ...
    private func handleOpenUrl(_ action: Action.OpenUrlAction, nodeId: String) async throws {
        NDLogger.d("ActionHandler", "Opening URL: \(action.url)")
    ...
    private func handleCustomAction(_ action: Action.CustomAction, nodeId: String) {
        NDLogger.d("ActionHandler", "Executing custom action: \(action.key)")
    ...
    private func handleNavigate(_ action: Action.NavigateAction, nodeId: String) {
        NDLogger.d("ActionHandler", "Navigating to: \(action.destination)")
    ...
    private func handleTrackEvent(_ action: Action.TrackEventAction, nodeId: String) {
        NDLogger.d("ActionHandler", "Tracking event: \(action.eventName)")
    ...
    private func handleCompositeAction(_ action: Action.CompositeAction, nodeId: String) async throws {
        NDLogger.d("ActionHandler", "Executing composite action with \(action.actions.count) sub-actions (\(action.executionMode))")
